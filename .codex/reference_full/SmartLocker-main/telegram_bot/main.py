...
from telegram_bot.config import load_telegram_bot_settings
...
def bookings_keyboard(bookings: list[dict], action: str) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="menu:home")])
...
async def main() -> None:
    ...
    dp = await create_dispatcher(api_client)
