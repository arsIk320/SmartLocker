...
LABEL_HELP = "ℹ️ Помощь"
LABEL_HOME = "⬅️ Назад в меню"
...
def help_text() -> str:
    return (
        ...
        "2. Выберите свою бронь кнопкой или отправьте номер варианта.\n"
        "3. Потом получайте QR через «🔐 Мои QR-коды».\n"
...
def booking_label(index: int, booking: dict) -> str:
    label = f"{index + 1}. {booking['house_name']} • {booking['door_name']}"
    return label[:64]
...
class MaxBotApplication:
    ...
    async def show_bound_bookings(
        ...
        *,
        ...
        prompt = "Выберите бронь для QR-кода:" if target_state == STATE_WAITING_FOR_QR_SELECTION else "Выберите бронь для отправки фото:"
    ...
    async def handle_message(self, incoming: IncomingMessage) -> None:
        ...
        if session.state in {
            ...
            await self._platform_client.send_message(
                recipient=incoming.recipient,
                text="📸 Отправьте одно фото лица сообщением MAX.",
                attachments=back_to_menu_attachments(),
            )
            return
