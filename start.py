...
def run_setup_wizard() -> None:
    ...
    configure_smtp = input("Настроить SMTP сейчас? [Y/n]: ").strip().lower() not in {"n", "no", "нет"}
    configure_travelline = input("Настроить TravelLine сейчас? [y/N]: ").strip().lower() in {"y", "yes", "да"}

    values: dict[str, str] = {}

    if configure_smtp:
        provider = input("Провайдер почты (gmail/outlook/yandex/custom) [gmail]: ").strip().lower() or "gmail"
        smtp_defaults = {
            ...
            "yandex": {"SMTP_HOST": "smtp.yandex.ru", "SMTP_PORT": "587", "SMTP_USE_TLS": "true"},
            "custom": {
        ...
        values["SMTP_HOST"] = prompt_value("SMTP_HOST", smtp_defaults.get("SMTP_HOST", os.getenv("SMTP_HOST", "")))
        values["SMTP_PORT"] = prompt_value("SMTP_PORT", smtp_defaults.get("SMTP_PORT", os.getenv("SMTP_PORT", "587")))
        values["SMTP_USERNAME"] = prompt_value("SMTP_USERNAME", os.getenv("SMTP_USERNAME", ""))
        values["SMTP_PASSWORD"] = prompt_value("SMTP_PASSWORD", os.getenv("SMTP_PASSWORD", ""), secret=True)
        values["SMTP_FROM_EMAIL"] = prompt_value("SMTP_FROM_EMAIL", os.getenv("SMTP_FROM_EMAIL", values.get("SMTP_USERNAME", "")))
        values["SMTP_USE_TLS"] = prompt_value("SMTP_USE_TLS", smtp_defaults.get("SMTP_USE_TLS", os.getenv("SMTP_USE_TLS", "true")))

    if configure_travelline:
        values["TRAVELLINE_CLIENT_ID"] = prompt_value("TRAVELLINE_CLIENT_ID", os.getenv("TRAVELLINE_CLIENT_ID", ""))
        values["TRAVELLINE_CLIENT_SECRET"] = prompt_value("TRAVELLINE_CLIENT_SECRET", os.getenv("TRAVELLINE_CLIENT_SECRET", ""), secret=True)
        values["TRAVELLINE_PROPERTY_ID"] = prompt_value("TRAVELLINE_PROPERTY_ID", os.getenv("TRAVELLINE_PROPERTY_ID", ""))
...
def print_startup_info() -> None:
    ...
    if os.getenv("SMTP_HOST"):
        ...
        print("- SMTP: not configured, email verification/reset will fail until SMTP_* variables are set")
