...
MODEL_DIR = Path(__file__).resolve().parents[3] / "runtime" / "models"
...
def _load_cv2():
    try:
        ...
        raise FaceMapError(
            "Dependency 'opencv-python' is not installed. Run: pip install -r requirements-face.txt"
        ) from exc
    return cv2
...
def _ensure_models() -> None:
    ...
    _download_if_missing(YUNET_MODEL_PATH, YUNET_MODEL_URL)
    _download_if_missing(SFACE_MODEL_PATH, SFACE_MODEL_URL)
